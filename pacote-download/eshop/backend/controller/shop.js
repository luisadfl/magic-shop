...
router.post("/create-shop", async (req, res, next) => {
    try {
        ...
        try {
            await sendMail({
                email: seller.email,
                subject: "Ative sua conta de anunciante",
                message: `Olá ${seller.name}, por favor clique no link para ativar sua conta de anunciante: ${activationUrl}`,
            });
            res.status(201).json({
                success: true,
                message: `por favor cheque seu email:- ${seller.email} para ativar sua conta de anunciante!`,
            });
        } catch (error) {
...
router.post(
    ...
    catchAsyncErrors(async (req, res, next) => {
        try {
            ...
            if (seller) {
                return next(new ErrorHandler("Conta já existe", 400));
            }
